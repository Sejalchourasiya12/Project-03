...
@WebServlet(name = "VehicleInsuranceListCtl", urlPatterns = { "/ctl/VehicleInsuranceListCtl" })
public class VehicleInsuranceListCtl extends BaseCtl {

    private static Logger log = Logger.getLogger(VehicleInsuranceListCtl.class);
    ...
    protected BaseDTO populateDTO(HttpServletRequest request) {
        ...
        if (vehicleType == null || vehicleType.trim().startsWith("--") || vehicleType.trim().isEmpty()) {
            dto.setVehicleType("");
        } else {
            dto.setVehicleType(vehicleType.trim());
        }

        String insuranceCompany = request.getParameter("insuranceCompany");
        if (insuranceCompany == null || insuranceCompany.trim().startsWith("--") || insuranceCompany.trim().isEmpty()) {
            dto.setInsuranceCompany("");
        } else {
            dto.setInsuranceCompany(insuranceCompany.trim());
        }
        ...
        try {
            ...
            if (OP_SEARCH.equalsIgnoreCase(op)) {
                ...
                if (ids != null && ids.length > 0) {
                    VehicleInsuranceDTO deleteBean = new VehicleInsuranceDTO();
                    for (String id : ids) {
    ...
    protected String getView() {
        return ORSView.VEHICLE_INSURANCE_LIST_VIEW;
    }
}