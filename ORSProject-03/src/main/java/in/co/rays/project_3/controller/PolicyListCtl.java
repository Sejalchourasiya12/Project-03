...
public class PolicyListCtl extends BaseCtl {
    ...
    protected BaseDTO populateDTO(HttpServletRequest request) {
        ...
        if (premiumAmount == null || premiumAmount.trim().startsWith("--")
                || premiumAmount.trim().isEmpty()) {
            dto.setPremiumAmount("");
        } else {
            dto.setPremiumAmount(premiumAmount.trim());
        }

        String policyStatus = request.getParameter("policyStatus");
        if (policyStatus == null || policyStatus.trim().startsWith("--")
                || policyStatus.trim().isEmpty()) {
            dto.setPolicyStatus("");
        } else {
            dto.setPolicyStatus(policyStatus.trim());
        }
        ...
        try {
            ...
            if (OP_SEARCH.equalsIgnoreCase(op)) {
                ...
                if (ids != null && ids.length > 0) {
                    PolicyDTO deleteBean = new PolicyDTO();
                    for (String id : ids) {
            ...
            request.setAttribute("nextListSize",
                    (next == null || next.size() == 0) ? "0"
                            : String.valueOf(next.size()));
