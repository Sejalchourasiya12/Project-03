...
@WebServlet(name = "SmartLightCtl", urlPatterns = { "/ctl/SmartLightCtl" })
public class SmartLightCtl extends BaseCtl {

    private static Logger log = Logger.getLogger(SmartLightCtl.class);

    // e.g. LGT001
    private static final Pattern LIGHT_CODE_PATTERN =
    ...
    protected boolean validate(HttpServletRequest request) {
        ...
        if (DataValidator.isNull(request.getParameter("lightCode"))) {
            request.setAttribute("lightCode",
                    PropertyReader.getValue("error.require", "Light Code"));
            pass = false;
        } else if (!LIGHT_CODE_PATTERN.matcher(
                request.getParameter("lightCode")).matches()) {
            request.setAttribute("lightCode",
                    "Light Code must be 3 Capital Letters followed by 3 Digits (e.g. LGT001)");
            pass = false;
        }
        ...
        if (DataValidator.isNull(request.getParameter("brightnessLevel"))) {
            request.setAttribute("brightnessLevel",
                    PropertyReader.getValue("error.require", "Brightness Level"));
            pass = false;
        }
    ...
    protected BaseDTO populateDTO(HttpServletRequest request) {
        ...
        String bVal = request.getParameter("brightnessLevel");
        dto.setBrightnessLevel((bVal != null && !bVal.trim().isEmpty() && !bVal.equals("0")) 
            ? Integer.parseInt(bVal.trim()) 
            : null);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (id > 0) {
            try {
                SmartLightDTO dto = model.findByPK(id);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (OP_SAVE.equalsIgnoreCase(op) || OP_UPDATE.equalsIgnoreCase(op)) {
                ...
                if (dto.getId() > 0) {
                    model.update(dto);
                    ServletUtility.setSuccessMessage("Smart Light Updated Successfully", request);
                    ServletUtility.setDto(dto, request);
                    ServletUtility.forward(getView(), request, response);
                    return;
                } else {
                    ...
                    request.getSession().setAttribute("successMessage",
                            "Smart Light Added Successfully");
                    ServletUtility.redirect(ORSView.SMART_LIGHT_CTL, request, response);
                    return;
                }
                ...
                ServletUtility.redirect(ORSView.SMART_LIGHT_LIST_CTL, request, response);
                return;
            }
