...
@WebServlet(name = "MobileStoreListCtl", urlPatterns = { "/ctl/MobileStoreListCtl" })
public class MobileStoreListCtl extends BaseCtl {

    private static Logger log = Logger.getLogger(MobileStoreListCtl.class);
    ...
    protected BaseDTO populateDTO(HttpServletRequest request) {
        ...
        if (brandName == null || brandName.trim().startsWith("--") || brandName.trim().isEmpty()) {
            dto.setBrandName("");
        } else {
        ...
        if (ramSize == null || ramSize.trim().startsWith("--") || ramSize.trim().isEmpty()) {
            dto.setRamSize("");
        } else {
            dto.setRamSize(ramSize.trim());
        }
        ...
        try {
            ...
            if (OP_SEARCH.equalsIgnoreCase(op)) {
                ...
                if (ids != null && ids.length > 0) {
                    MobileStoreDTO deleteBean = new MobileStoreDTO();
                    for (String id : ids) {
    ...
    protected String getView() {
        return ORSView.MOBILE_STORE_LIST_VIEW;
    }
}