...
@WebServlet(name = "CloudStorageListCtl", urlPatterns = { "/ctl/CloudStorageListCtl" })
public class CloudStorageListCtl extends BaseCtl {

    private static Logger log = Logger.getLogger(CloudStorageListCtl.class);
    ...
    protected BaseDTO populateDTO(HttpServletRequest request) {
        ...
        if (fileSize == null || fileSize.trim().startsWith("--") || fileSize.trim().isEmpty()) {
            dto.setFileSize("");
        } else {
            dto.setFileSize(fileSize.trim());
        }

        String storageType = request.getParameter("storageType");
        if (storageType == null || storageType.trim().startsWith("--") || storageType.trim().isEmpty()) {
            dto.setStorageType("");
        } else {
            dto.setStorageType(storageType.trim());
        }
        ...
        try {
            ...
            if (OP_SEARCH.equalsIgnoreCase(op)) {
                ...
                if (ids != null && ids.length > 0) {
                    CloudStorageDTO deleteBean = new CloudStorageDTO();
                    for (String id : ids) {
    ...
    protected String getView() {
        return ORSView.CLOUD_STORAGE_LIST_VIEW;
    }
}